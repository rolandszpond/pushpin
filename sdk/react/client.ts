/**
 * Pushpin Client SDK (React)
 *
 * Works unmodified in React Native too — it only uses the standard
 * `WebSocket` global and React hooks, neither of which are DOM-specific.
 *
 * import { PushpinClient, usePushpinChannel } from './client'
 *
...
type UsePushpinChannelOptions = {
    client: PushpinClient
    /** Required for `private-`/`presence-` channels unless the client has one. */
    authorizer?: Authorizer
}

type AuthFailure = { channel: string; error: string; fatal: boolean }

type UsePushpinChannelReturn = {
    on: (event: string, handler: EventHandler) => void
    off: (event: string, handler?: EventHandler) => void
    status: StatusType
    messages: Array<{ event: string; data: unknown; timestamp: number }>
    /**
     * Set when a private channel was refused. `fatal` means it has stopped
     * trying — render "you don't have access" rather than a spinner that will
     * never resolve.
     */
    authError: AuthFailure | null
}

export function usePushpinChannel(
    channelName: string,
    { client, authorizer }: UsePushpinChannelOptions,
): UsePushpinChannelReturn {
    const ch = client.channel(channelName, { authorizer })
    const [status, setStatus] = useState<StatusType>(client.getStatus(channelName))
    const [messages, setMessages] = useState<Array<{ event: string; data: unknown; timestamp: number }>>([])
    const [authError, setAuthError] = useState<AuthFailure | null>(null)

    const on = useCallback((event: string, handler: EventHandler) => { ch.on(event, handler) }, [ch])
    const off = useCallback((event: string, handler?: EventHandler) => { ch.off(event, handler) }, [ch])

    useEffect(() => {
        const onAny = ({ event, data }: { event: string; data: unknown }) => {
            setMessages((prev) => [{ event, data, timestamp: Date.now() }, ...prev].slice(0, 100))
        }
        const onAuthFailed = (data: unknown) => setAuthError(data as AuthFailure)

        ch.on('*', onAny)
        ch.on('pushpin:auth_failed', onAuthFailed)

        // Poll status — lightweight, only while the component is mounted.
        // (`pushpin:connected` is handled inside the client and never reaches
        // a channel handler, so this poll is what surfaces the status here.)
        const statusInterval = setInterval(() => setStatus(client.getStatus(channelName)), 1000)

        return () => {
            ch.off('*', onAny)
            ch.off('pushpin:auth_failed', onAuthFailed)
            clearInterval(statusInterval)
        }
    }, [ch, client, channelName])

    return { on, off, status, messages, authError }
}
